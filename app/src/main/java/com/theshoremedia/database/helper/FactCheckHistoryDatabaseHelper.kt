...
class FactCheckHistoryDatabaseHelper {
    ...
    fun getUnreadNews(callBack: (items: List<FactCheckHistoryModel>) -> Unit) {
        handler?.post {
            if (AppDatabase.appDatabase == null) return@post
            callBack.invoke(
                AppDatabase.appDatabase!!.factChecksHistoryDao()!!.getUnreadNews()
                    ?: arrayListOf()
            )
        }
    }

    fun getFavouriteNews(callBack: (items: List<FactCheckHistoryModel>) -> Unit) {
        handler?.post {
            if (AppDatabase.appDatabase == null) return@post
            callBack.invoke(
